...
bool isValidUserBoard(string &userBoardText, int rowsAndCols);
...
Boggle setUpBoard(Lexicon& dictionary, int rowsAndCols) {
    ...
    if (!getYesOrNo("Do you want to generate a random board? ")) {
        while(!isValidUserBoard(userBoardText, rowsAndCols)) {
 ...
 */
bool isValidUserBoard(string& userBoardText, int rowsAndCols) {
    userBoardText = getLine("Type the 25 letters to appear on the board: ");
    if (userBoardText.length() != rowsAndCols*rowsAndCols) return false;
...
void printResultOfGame(Boggle& board) {
    ...
    if (scoreDiff > 0) {
        ...
        BoggleGUI::playSound("yeah-right.wav");
...
void printHumanWordMessage(bool validWord, string humanWord) {
    ...
    if (validWord) {
        ...
        BoggleGUI::playSound("denied.wav");
    }
    printMessageToConsoleAndGUI(message);
}
