 ...
 * words. Lecture slides, the Stanford CS 106B textbook,
...
Boggle::Boggle(Lexicon& dictionary, string boardText, int rowsAndCols) {
    ...
    if (boardText.empty()) { // user has selected a random board
        shuffle(CUBES, numRowsAndCols^2);
        for (int i = 0; i < numRowsAndCols; i++) {
            for (int j = 0; j < numRowsAndCols; j++) {
                boardGrid.set(i, j, CUBES[numRowsAndCols*i+j][randomInteger(0,5)]);
            }
        }
    } else {
        for (int i = 0; i < numRowsAndCols; i++) {
...
bool Boggle::humanWordSearch(string word) {
    ...
    highlightedLetters.fill(0); // no highlighted letters
...
Grid<int> Boggle::availableSpaces(int row, int col) {
    ...
    if (row == -1 && col == -1) { // haven't started board searching yet
        availableSpaces.fill(1); // all spaces are neighbors
    } else {
